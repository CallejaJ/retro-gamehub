...
import { addComment, likeComment } from "./actions";
import { Footer } from "@/components/footer";
import type { Comment, Score } from "@/lib/supabase";
...
const normalizeGameName = (gameName: string) =>
  gameName.replace(/ Classic| Retro/g, "");

export function LeaderboardClient({ comments, scores }: LeaderboardClientProps) {
  ...
  const filteredLeaderboard =
    selectedFilter === "all"
      ...
      : scores.filter(
          (player) =>
            normalizeGameName(player.game_name) ===
            normalizeGameName(selectedFilter)
        );
  ...
  const filteredComments =
    selectedFilter === "all"
      ...
      : comments.filter(
          (comment) =>
            normalizeGameName(comment.game_name) ===
            normalizeGameName(selectedFilter)
        );
                ...
                <CardContent className='p-3 sm:p-6'>
                  ...
                  >
                    ...
                    <Button
                      ...
                      disabled={
                        isPending || !newCommentText.trim() || !userName.trim()
                      }
                    >
                          ...
                          <div className='flex items-start justify-between mb-2 sm:mb-3'>
                            ...
                            <div className='flex items-center space-x-2 sm:space-x-3 flex-shrink-0'>
                              ...
                              <button
                                ...
                                disabled={isPending}
                                className='flex items-center space-x-1 text-red-400 hover:text-red-300 transition-all duration-200 hover:scale-110 font-mono text-xs sm:text-sm bg-red-500/20 px-1 sm:px-2 py-1 rounded border border-red-500/30 disabled:opacity-50'
